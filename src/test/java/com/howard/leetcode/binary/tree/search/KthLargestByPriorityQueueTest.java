...
@DisplayName("利用PriorityQuery来实现返回第K大元素的值")
class KthLargestByPriorityQueueTest {

    private KthLargestByPriorityQueue solution;

    @ParameterizedTest
    @MethodSource("provider")
    @DisplayName("测试")
    void add(int k, int[] nums, List<Node> list) {
        solution = new KthLargestByPriorityQueue(k, nums);
        list.forEach(result -> {
            int kVal = solution.add(result.add);
